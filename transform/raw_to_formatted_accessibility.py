...
def format_accessibility_data(**kwargs) -> str:
    """Run the AccesLibre Spark formatting job reading from / writing to S3."""
    day = current_day()
    raw_path = f"s3a://{RAW_BUCKET}/acces_libre/establishments/{day}/establishments.json"
    output_path = f"s3a://{FORMATTED_BUCKET}/acces_libre/establishments/{day}"

    print(f"Submitting Spark formatting (S3) for accessibility data...")
    ...
    print(f"Accessibility formatting complete -> {output_path}")
    return output_path
